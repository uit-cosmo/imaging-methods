...
ccf = True
name = (
    "grid_ccf{}{}.eps".format(refx, refy)
    if ccf
    else "grid_ca{}{}.eps".format(refx, refy)
)
...
pd = ve.estimate_velocities_for_pixel(
    refx, refy, ve.CModImagingDataInterface(ds_short), estimation_options=eo
)
vx, vy = pd.vx, pd.vy
print("Velcities estimated with 3TDE v={:.2f}, w={:.2f}".format(vx, vy))
deltax = (
    ds_corr.R.isel(x=refx, y=refy).values - ds_corr.R.isel(x=refx - 1, y=refy).values
)
deltay = (
    ds_corr.Z.isel(x=refx, y=refy).values - ds_corr.Z.isel(x=refx, y=refy - 1).values
)
