...
blobs = [get_blob(vx=vx, vy=vy, posx=np.random.uniform(0, Lx), posy=Ly, lx=lx, ly=ly, t_init=np.random.uniform(0, T), theta=theta) for _ in range(num_blobs)]
...
plot_ccfs_grid(ds, ax, refx, refy, [6, 5, 4, 3, 2], [0, 1, 2, 3], delta=delta, plot_tau_M=True, vx=vx, vy=vy, lx=lx, ly=ly, theta=theta)
...
pd = ve.estimate_velocities_for_pixel(refx, refy, ve.CModImagingDataInterface(ds), estimation_options=eo)
vx, vy = pd.vx, pd.vy
