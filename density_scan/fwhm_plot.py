...
for refx in range(9):
    for refy in range(10):
        ...
        if os.path.exists(results_file_name):
            full_results[refx * 10 + refy] = ph.ScanResults.from_json(
...
for gf_key, group in gf_groups.items():
    ...
    ax[0].plot(radial_values, avg_lr, label=r"$F_{GW}=$" f"${avg_gf:.2f}$")
...
for gf_key, group in gf_groups.items():
    ...
    ax[0].plot(radial_values, avg_lz, label=r"$F_{GW}=$" f"${avg_gf:.2f}$")
